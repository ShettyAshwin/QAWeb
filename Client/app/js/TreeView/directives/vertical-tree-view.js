    ...
    f.module("angularTreeview", []).directive("treeModel", function ($compile) {
        return{restrict: "A", link: function (b, h, c) {
            ...
            var a = c.treeId, g = c.treeModel, e = c.nodeLabel || "label", d = c.nodeChildren || "children",t= c.nodeType, e = '<ul>' +
                '<li data-ng-repeat="node in ' + g + '"><i class="collapsed" data-ng-show="node.type !=asset && node.' + d + '.length && node.collapsed" data-ng-click="' + a + '.selectNodeHead(node)"></i>' +
                '<i class="expanded" data-ng-show="node.type !=asset && node.' + d + '.length && !node.collapsed" data-ng-click="' + a + '.selectNodeHead(node)"></i>' +
                '<i class="normal" data-ng-show="node.type == asset" ></i> ' +
